...
export class StyleSelectorComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    const tmp = this.categories.map((cat) =>
      this.styles.filter((style) => style.category._id === cat._id)
    );
    const max = tmp
      .map((styles) => styles.length)
      .reduce((prev, current) => Math.max(prev, current));
    tmp.forEach((styles, i) =>
      styles.forEach(
        (style) => (style.colour = this.colours[i % this.colours.length])
      )
    );
    tmp.forEach((styles) =>
      styles.unshift(...Array.from({ length: max - styles.length }, () => null))
    );
    this.matrix = this.categories.map(() => []);
    tmp.forEach((styles, i) =>
      styles.forEach((style, j) => (this.matrix[j][i] = style))
    );
